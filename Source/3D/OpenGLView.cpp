...
#include	"../Interface/InputReader.h"
...
OpenGlView :: OpenGlView ( const char * theName, HINSTANCE hInst, Camera * theCamera, int theWidth, int theHeight, 
						  int bpp, int toFullScreen ) : View ( theName ), events ( "events queue" )
{
//	(*sysLog) << "Creating OpenGlView" << logEndl;
	...
	if(toFullScreen == 1)
		...
		fullScr = false;
	createWindow    ( theWidth, theHeight, bpp, fullScr, 32 );
}
...
bool	OpenGlView :: getMouseState ( MouseState& state )
{
	...
	state.setState (mouseX - lastMouseX, mouseY - lastMouseY, 0, mouseLeft != 0, mouseRight != 0, mouseMiddle != 0, 0);
...
bool	OpenGlView :: createWindow ( int theWidth, int theHeight, int bpp, bool isFullScreen, int depthBits )
{
	...
	POINT p;
	GetCursorPos(&p);
	mouseX = lastMouseX = p.x;
	mouseY = lastMouseY = p.y;
...
void	OpenGlView :: apply ( const Camera& camera )
{
	...
	camera.buidProjectionPerspectiveMatrix(proj);
	glLoadMatrixf(proj);
	...
	switch ( msg )												// evaluate window message
	{
		...
		case WM_KEYDOWN:
			if ( wParam >= 0 && wParam < 256 )
				view -> keys [translateKey ( (int)wParam )] = 1;
		...
		case WM_KEYUP:
			if ( wParam >= 0 && wParam < 256 )
				view -> keys [translateKey ( (int)wParam )] = 0;
		...
		case WM_MOUSEMOVE:
			view->lastMouseX = view ->mouseX;
			view->lastMouseY = view ->mouseY;
